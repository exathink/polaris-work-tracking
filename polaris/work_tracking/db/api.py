...
def sync_work_items(work_items_source_key, work_item_list, epic=None, join_this=None):
    # About the epic parameter: This is a work item of type epic. This is passed when we want to attach\
    # epic_id to the work items passes along. When setting epic_id, we will have the epic_id column \
    # included in temp table (unlike general sync case). For all the work items in the list we will set \
    # epic_id to id of epic(work_item) passed.
    ...
    if len(work_item_list) > 0:
        with db.orm_session(join_this) as session:
            ...
            if epic is not None:
                ...
                # Insert into work_items_temp, work items from work_items table which have epic_key \
                # same as current epic, but do no exist in work_items_temp
                ...
                session.connection().execute(
                    work_items_temp.insert().from_select(
                        ...
                        select(
                            [
                                literal(uuid.uuid4()).label('key'),
                                work_items.c.work_items_source_id,
                                literal(last_sync).label('last_sync'),
                                literal(None).label('epic_id'),
                ...
                session.connection().execute(
                    upsert.on_conflict_do_update(
                        ...
                        set_=dict(
                            ...
                            epic_id=upsert.excluded.epic_id,
