...
from polaris.messaging.messages import WorkItemsSourceCreated, WorkItemsSourceUpdated
...
class TestConnectorsTopicSubscriber:
    class TestRefreshConnectorProjects:
        ...
        def it_returns_a_valid_response_when_ew_project_is_added(self, setup_connectors):
            project = {
                'id': 24543756,
                'description': '',
    ...
    def it_returns_a_valid_response_when_existing_project_is_updated(self, setup_connectors):
        project = {
            'id': 24543756,
            'description': '',
            'name': 'Exathink Alliance Test',
        ...
        with patch(
            ...
            messages = subscriber.dispatch(mock_channel, refresh_connector_projects_message)
            assert len(messages) == 2
            publisher.assert_topic_called_with_message(WorkItemsTopic, WorkItemsSourceUpdated, call=1)
